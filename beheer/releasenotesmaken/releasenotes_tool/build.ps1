# build.ps1 - Bouw de NLCS Release Notes tool tot een losse .exe
#
...
& $py -m pip install --quiet --upgrade pyinstaller requests
...
Remove-Item -Recurse -Force build, "NLCS-Releasenotes.spec", __pycache__ -ErrorAction SilentlyContinue

Write-Host ""
Write-Host "Klaar. De executable staat in:  dist\NLCS-Releasenotes.exe" -ForegroundColor Green
Write-Host "Tussenmap 'build\' en 'NLCS-Releasenotes.spec' zijn opgeruimd." -ForegroundColor Green
