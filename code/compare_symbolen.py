"""
Compare NLCS 5.0 vs 5.1 symbolen tables, split by hoofdgroep.
Output: ontwikkeling/heroverweging-5-1/{HOOFDGROEP}/symbolen-vergelijking-{HOOFDGROEP}.html
"""
import csv
import io
import os
...
FILE_51 = r"tabellen\publicatie\NLCS_Query_Symbolen-concept-5.1.csv"
FILE_50 = r"C:\Users\100289\OneDrive - CROW\Documents\GitHub\NLCSmain\NLCS\tabellen\publicatie\5.02-symbolen.csv"
...
def main():
    ...
    # Group 5.1 by sbibliotheek -> hoofdgroep (strip 'S' prefix)
    sbib_idx_51 = headers_51.index("sbibliotheek")
    groups_51 = defaultdict(list)
    for row in rows_51:
        sbib = row[sbib_idx_51].strip() if sbib_idx_51 < len(row) else ""
        hg = sbib[1:] if sbib.startswith("S") else sbib  # SAL -> AL
        groups_51[hg].append(row)

    # Group 5.0 by sbibliotheek -> hoofdgroep
    sbib_idx_50 = headers_50.index("sbibliotheek")
    id_idx_50 = headers_50.index("id")
    groups_50 = defaultdict(dict)
    for row in rows_50:
        sbib = row[sbib_idx_50].strip() if sbib_idx_50 < len(row) else ""
        hg = sbib[1:] if sbib.startswith("S") else sbib
    ...
    for group in all_groups:
        ...
        title = f"Symbolen vergelijking 5.0 vs 5.1 — {group}"
        ...
        filename = f"symbolen-vergelijking-{group}.html"
