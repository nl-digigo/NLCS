...
* De volgende controles worden uitgedraaid bij gebruik van de nlcs-exporter:
  ...
  * [query: controle_lijntypes_zonder_objectRelatie](https://github.com/nl-digigo/NLCS/blob/acceptance/code/nlcs/controlesdatabase/controle_lijntypes_zonder_objectRelatie.rq) controleert of alle lijntypes zijn gekoppeld aan een object, zo niet het lijntype verwijderen. Controle andersom hoeft niet, de publicatie geeft geen waarde voor het lijntype bij het object, als deze niet bestaat. In de beheeromgeving zijn de lijntypes niet gekoppeld aan de objecten.
...
2. Controle ID's
- [ ] Controleer (in de outputtabellen) of alles een ID heeft & geen dubbele ID. Helaas worden de ID's handmatig aangemaakt, de linked data beheeromgeving heeft stabiele URI's niet automatische ID nummering.
...
3. Controle symbolen
- [ ] Controleer of de symbolen goed werken in AutoCAD & geoptimaliseerd zijn
