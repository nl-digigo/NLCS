...
<figure>
<img src="../../media/laagopbouwnlcs.PNG" alt="Grafische weergave van laagopbouw NLCS: (ordening) STATUS- DISCIPLINE– HOOFDGROEP– (objecten) OBJECT _ [SUBOBJECT 01] _ [SUBOBJECT 02] _ [SUBOBJECT 03] _ [SUBOBJECT 04]_[SUBOBJECT 05] – [BEWERKING] – (tekeneigenschappen) ELEMENT–[SCHAAL]">
...
<dfn>DISCIPLINE</dfn>
<d>De DISCIPLINE geeft het vakgebied aan waarop het model betrekking heeft en vormt voor de gebruiker een mogelijkheid voor de ordening of filtering van informatie in een CAD model.<br><br>
...
1. NLCS schrijft niet voor welke objecten of hoofdgroepen binnen een discipline vallen. Gebruikers maken hier organisatie-specifieke afspraken over.  
2. De DISCIPLINES die in NLCS zijn opgenomen, zijn weergegeven in onderstaande tabel.
3. De DISCIPLINE “XX” mag alleen worden toegepast in combinatie met de hoofdgroep “AL”.
...
### HOOFDGROEP
<dfn>HOOFDGROEP</dfn>
<d>Een HOOFDGROEP is een logische verzameling OBJECTEN. Soms betreft dit een systeem met objecten die in samenhang ontworpen worden, zoals in het geval van de hoofdgroep RI (RIoleringen), soms een verzameling systemen, zoals hoofdgroep KL (Kabels en Leidingen), soms een verzameling objecten die logisch is vanuit een inventarisatie zoals IE (Inrichtringselementen) waarin alle bovengrondse objecten staan die in de weg kunnen staan bij herinrichting.<br><br>
...
1. De HOOFDGROEPEN die in NLCS worden onderscheiden, worden gepubliceerd als onderdeel van de standaard. De actuele lijst met hoofdgroepen [staat hier](https://github.com/nl-digigo/NLCS/tree/main/tabellen/actueel)
2. Er is een splitsing gemaakt tussen HOOFDGROEPEN die deel uitmaken van de ‘Buitenruimte’ en HOOFDGROEPEN die deel uitmaken van ‘Constructies’. De reden hiervoor is, dat het gaat om twee wezenlijk verschillende categorieën met verschillende gebruiksmogelijkheden voor CAD modellen.
...
3. De HOOFDGROEP “AL” mag alleen worden toegepast in combinatie met de STATUS “X” en DISCIPLINE “XX”.
4. De HOOFDGROEP “ZZ” bevat objecten die voor alle HOOFDGROEPEN voorkomen. De objecten worden op de gewenste HOOFDGROEP geplaatst, HOOFDGROEP “ZZ” wordt niet gebruikt als HOOFDGROEP voor het plaatsen van objecten.  
...
<dfn>OBJECT</dfn>
<d>Een OBJECT is een gestandaardiseerd element in een CAD-model of -tekening dat een bouwkundig, civieltechnisch of installatietechnisch verschijnsel representeert. Een OBJECT kan uit meerdere SUBOBJECTEN bestaan.<br><br>

<dfn>SUBOBJECT</dfn>
<d>Een SUBOBJECT geeft een decompositie (onderdeel) of specialisatie (soort) weer van het bovenliggende (SUB)OBJECT, waarbij de specialisatie ook bijvoorbeeld de aanduiding kan zijn van een materiaal, vorm of afmeting van het OBJECT. <br><br>
...
#### BEWERKING
<dfn>BEWERKING</dfn>
<d>Een BEWERKING beschrijft het uit te voeren werk aan het NLCS-Object.<br><br>

<ol>
<li>Aan ieder OBJECT of SUBOBJECT kan optioneel een BEWERKING worden toegevoegd.
<li>Alle NLCS-objecten met BEWERKINGEN MOETEN de STATUS ‘NIEUW’ of ‘TIJDELIJK’ krijgen.
</ol>
