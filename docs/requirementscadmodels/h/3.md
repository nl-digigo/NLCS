...
<p id="footnote-3">[3] In de regel geldt dat lijnen van het ELEMENT GD (Geometrie in doorsnede) moeten worden getekend met één pen dikker (binnen dezelfde lijngroep) dan in de NLCS objectentabellen is aangegeven voor het ELEMENT G (Geometrie in aanzicht). Ook hier geldt dat een andere lijndikte binnen dezelfde lijngroep gekozen mag worden als dit de leesbaarheid van de tekening ten goede komt.</p>
