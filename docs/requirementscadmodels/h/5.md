...
1. Een ELEMENT geeft weer wat er in de laag is getekend: een of meer [grafische representaties](#representatie-van-objecten) van een (SUB)OBJECT, zoals geometrie, arcering of symbool; of een hulpelement voor duiding van de tekening zoals een tekst of maatvoering.
2. In de NLCS worden de volgende elementen onderscheiden:
...
3. Als in de laagnaam alleen de HOOFDGROEP voorkomt zonder een OBJECT uit deze hoofdgroep, mogen alleen MAATVOERING of TEKST gebruikt worden als element. De algemene objecten uit hoofdgroep ZZ (aanpijling, afbreeklijn, hulplijn et cetera) mogen ook in de laagnaam voorkomen zonder OBJECT uit de hoofdgroep. Een symbool of arcering is altijd gekoppeld aan een OBJECT of SUBOBJECT. 
4. Bepaalde (SUB)OBJECTEN kunnen in een tekening naar keuze worden weergegeven als Geometrie (G), Arcering (A) of Symbool (S). In de NLCS objectentabellen zijn deze (SUB)OBJECTEN herkenbaar aan de vermeldingen G/A, G/S of G/A/S in de kolom “Element” (G/A: kan Geometrie of Arcering zijn; G/S: kan Geometrie of Symbool zijn; G/A/S: kan Geometrie of Arcering of Symbool zijn). 
